...
# SwarnUnadjLRT_Results <- SwarnSeq::SwarnUnadjLRT(CountData = CountData, norm.method = "DEseq.norm", group = group, CellCluster = CellCluster)

# SwarnAdjLRT_Results <- SwarnSeq::SwarnAdjLRT(CountData = CountData, norm.method = "DEseq.norm", group = group, CellCluster = CellCluster)
```
...
```{r}
# SwarnClass <- SwarnSeq::SwarnClassDE(results = SwarnAdjLRT_Results, alpha = o.01)
...
# ExtractedAdjNormData <- SwarnSeq::ExtDEGAdjNormData(CountData = counts_data_matrix, norm.method = "DEseq.norm", group = group, RNAspike.use = T, CE.range = c(0.01, 0.5))
```
