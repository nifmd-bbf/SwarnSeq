...
# SpikeInData <- SingleCellExperiment(assays = list(TestData$SpikeCounts), rowData = TestData$SpikeConc)
...
top_genes <- swarnTopTags(results = SwarnAdjLRT_Results, m = 50)
```
...
clusters <- SwarnSeqToyData$clusters[c(1:50, 350:399)]
testData <- SingleCellExperiment(assays = list(counts = data), colData = data.frame(clusters = clusters, groups = groups))
# SpikeInData <- SingleCellExperiment::SingleCellExperiment(assays = list(TestData$SpikeCounts), rowData = TestData$SpikeConc)
# Make the spike-in single cell experiment object like this.
