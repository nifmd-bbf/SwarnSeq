...
ZINBEM <- function(counts_data, group, CellCluster, CellAuxil = NULL, weights = NULL, muoffset = NULL, phioffset = NULL, maxit = NULL, eps = NULL) {
    ...
    gradNegBin <- function(par, Ycounts, X, Z, weights, offsetx, offsetz) {
        ...
        logit_deriv <- stats::make.link("logit")$mu.eta(etaz)
    ...
    for (i in seq_len(length.out = m)) {
        ...
        model_zero <- glm.fit(x = Z, y = Y0, family = stats::binomial(link = "logit"), weights = weights, offset = offsetz)
        ...
        probi <- probi / (probi + (1 - probi) * stats::dnbinom(0, size = start$theta, mu = mui))
        ...
        tryCatch(
            expr = {
                while (abs(ll_old - ll_new) > eps) {
                    ...
                    model_zero <- glm.fit(x = Z, y = probi, family = stats::binomial(link = "logit"), weights = weights, offset = offsetz)
        ...
        if (!is.na(ll_new) && !is.na(ll_old)) {
            if (abs(ll_old - ll_new) <= eps & is.null(start$iter)) {
                ...
                model_count <- glm.fit(x = X, y = Y, family = stats::poisson(), weights = weights, offset = offsetx)
                c <- c(model_count$coefficients, model_zero2$coefficients)
                fit <- stats::optim(par = c(c,1), fn = function(par) zinb_loglikfun(par, Y, X, Z, weights, offsetx, offsetz), gr = function(par) gradNegBin(par, Y, X, Z, weights, offsetx, offsetz), method = "BFGS", control = list(), hessian = FALSE)
                start <- list()
        ...
        phi <- stats::plogis(Z %*% start$zero + offsetz)
        ...
        pval <- exp(stats::pchisq(lrt.stat, df = 1, lower.tail = FALSE, log.p = TRUE))
        # Differential zero inflation
        start$zero0 <- start$zero
        # Under null hypothesis
        start$zero0[2] <- 0
        phi0 <- stats::plogis(Z %*% start$zero0 + offsetz)
        ...
        pval.phi <- exp(stats::pchisq(lrt.stat.phi, df = 1, lower.tail = FALSE, log.p = TRUE))
        # DE stat
